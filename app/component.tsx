...
interface InputProps extends Extendable {
    id: string
}
...
const Button:React.FC<Extendable> = ({text, className=""}) => {
    return(
...
export const ButtonPrimaryLarge:React.FC<Extendable> = ({text, className=""}) => {
    return (
...
export const ButtonPrimarySmall:React.FC<Extendable> = ({text, className=""}) => {
    return (
...
export const ButtonSecondaryLight:React.FC<Extendable> = ({text, className=""}) => {
    return (
...
export const ButtonSecondaryDark:React.FC<Extendable> = ({text, className=""}) => {
    return (
        <Button text={text} className={twMerge(`${typography.button_S} text-main_purple bg-[white] hover:bg-[white] py-2`, className)}></Button>  
    )
}

export const ButtonDestructive:React.FC<Extendable> = ({text, className=""}) => {
    return (
...
export const SubtaskCheckbox:React.FC<InputProps> = ({text, className="", id}) => {
    return(
        <div className={twMerge("p-3 bg-very_dark_grey rounded-md hover:bg-main_purple cursor-pointer", className)}>
            <div className="flex items-center relative p-0">
                ...
                <label htmlFor={id} className={`${typography.heading_S} mt-[2px] ml-4 peer-checked:line-through peer-checked:opacity-50`}>{text}</label>
            </div>
        </div>
    )
}

export const TextField:React.FC<InputProps> = ({text, className="", id}) => {
    return(
        <div className={twMerge("flex flex-col", className)}>
            <label htmlFor={id}>{text}</label>
            <input type="text" id={id} className="border bg-opacity-0 bg-[white] border-medium_grey border-op focus:outline-none"></input>
        </div>
    )
